...
async function handleAdicionartwitch(message, args) {
    ...
    data.streamers.push({ type: 'twitch', name: streamerNameOrUrl });

    // Salva de volta
    fs.writeFileSync(streamersFilePath, JSON.stringify(data, null, 2));

    message.reply(`Streamer ${streamerNameOrUrl} adicionado com sucesso na Twitch!`);
}

module.exports = { handleAdicionartwitch };